...
//    }
    ...
    protected void fillRootElement(final EObject semanticModel, final Diagram notationModel, final GModelRoot newRoot) {
        ...
        createConstraintLegend(emfFeatureModel.getConstraints(), layoutConstraintLegend(gRoot.gNode));
    ...
    private FeatureSubtreeResult buildFeatureSubtree(Feature feature, boolean isRoot) {
        ...
        String primaryCss = isRoot
                ...
                : CardinalityType.of(featureCardinality).value();
        GNode gNode = createNode(feature, currentPosition, primaryCss);

        FeatureImplementationTypes secondary = AttributeKeysUtils.getFeatureType(feature);
        if (secondary != FeatureImplementationTypes.NONE) {
    ...
    private FeatureSubtreeResult buildNodeSubtree(GroupNode groupNode) {
        ...
        gNode = createNode(
                groupNode,
                currentPosition,
                resolveGroupType(groupNodeCardinality).value());
        TreeNode currentNode = new TreeNode(gNode.getId());
    ...
    private NodeType resolveGroupType(Cardinality c) {
        if (CardinialityUtils.isOr(c)) return NodeType.OR_NODE;
        if (CardinialityUtils.isXor(c)) return NodeType.XOR_NODE;
        if (CardinialityUtils.isAnd(c)) return NodeType.AND_NODE;
    ...
    protected GNode createNode(final Identifiable identifiable, final GPoint gPosition, final String cssType) {
        ...
        if (identifiable instanceof Feature) {
            nodeBuilder.add(new GLabelBuilder(FeatureModelLables.EDITABLE_LABEL)
...
//        	nodeBuilder.size(GraphUtil.dimension(groupNodeWidth, groupNodeHeight));
    ...
    protected void createEdge(
            final Identifiable source, final Identifiable target, final GNode gSource, final GNode gTarget) {
        EdgeType edgeType;
        ...
        if (target instanceof Feature) {
            ...
            if (CardinialityUtils.isOr(featureGroupCardinality) || CardinialityUtils.isXor(featureGroupCardinality)) {
                edgeType = EdgeType.BASIC_EDGE;
            } else if (CardinialityUtils.isOptional(featureCardinality)) {
                edgeType = EdgeType.OPTIONAL_EDGE;
            } else if (CardinialityUtils.isMandatory(featureCardinality)) {
    ...
    protected void layoutFeatureTree(final FeatureSubtreeResult gRoot) {
        ...
        FeatureTreeLayouter.computePositions(
                FeatureTreeLayouter.mapGNodeToTreeNode(gRoot.gNode),
                0,
                horizontalGap,
                verticalGap,
                nodeWidth,
...
//            gNode.setPosition(GraphUtil.point(node.x, node.y));
    ...
    protected GNode createConstraintLegend(final List<Constraint> constraints, final GPoint coords) {
        ...
        legend.setPosition(GraphUtil.point(
                legend.getPosition().getX() + (nodeWidth / 2) - (legendWidth / 2),
    ...
    public GLabel createConstraintLabel(final Constraint constraint, final int id) {
        ...
        GLabel gConstraint = new GLabelBuilder(FeatureModelLables.EDITABLE_LABEL)
                .id(gId)
